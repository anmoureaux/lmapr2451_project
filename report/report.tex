...
\end{tabular}
\end{center}
\subsection{Convergence with respect to the cut-off energy (\texttt{ecut})}
...
\subsection{Convergence with respect to the number of $k$-points (\texttt{ngkpt})}
...
Finally, the structural parameters of the unit cell will be optimized automatically by \texttt{Abinit}. This is done using the input file \texttt{1522\_1\_strucRel.abi} (\autoref{Abi4}). We use the following parameters 
\begin{center}
\begin{tabular}{lll}
\texttt{ecut} & \texttt{40.0} &\\
\texttt{toldff} & \texttt{1.0d-6} &\\
...
\end{tabular}
\end{center}
and 4 datasets going from \texttt{ngkpt} = \texttt{[2 2 2]} to \texttt{ngkpt} = \texttt{[5 5 5]}. The convergence is obtained with \texttt{ngkpt} = \texttt{[3 3 3]}, as depicted on \autoref{fig:acellConv}.
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/acellConv}
\caption{(a) Total energy and (b) total energy per atom with respect to the number of $k$-points in the $k$-points grid, with relaxation of the (volume of the) unit cell. Linear interpolation for visual guidance only.}
\label{fig:acellConv}
\end{figure}
However, the converged value for \texttt{acell} will be the one corresponding to \texttt{ngkpt} = \texttt{[4 4 4]}, as it is the most accurate iteration that isn't likely to break the symmetry of the cell. It is equal to \begin{center}\texttt{6.45315083607590E+00  8.44810720073889E+00  1.03002947415949E+01},\end{center} which corresponds to \begin{center}\texttt{3.41486036030633278 4.470545805026401531 5.450681241775095387 Angstrom}.\end{center}
\subsection{Convergence with respect to both the number of $k$-points and the broadening factor}
\subsection{Summary}
\begin{center}
\begin{tabular}{l|c}
\textbf{Parameter} & \textbf{Converged value} \\
\hline
\texttt{ecut} & \texttt{40.0} [Ha] \\
\hline
\texttt{ngkpt} & \texttt{[4 4 4]}\\
\hline
\texttt{acell} & \texttt{3.41586 4.47055 5.45068}\\
\hline
\texttt{acell} (with \texttt{tsmear}) & VALUE
\end{tabular}
\end{center}
\newpage
